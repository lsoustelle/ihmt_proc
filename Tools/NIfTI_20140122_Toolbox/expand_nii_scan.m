...
function expand_nii_scan(filename, img_idx, newpath)
   ...
   for i=img_idx
      ...
      temp = nii_i.fileprefix;  % retrieve actual fielprefix (= filename)
      temp = strsplit(nii_i.fileprefix,filesep);
      nii_i.fileprefix = temp{end};
      fn = [nii_i.fileprefix '_' sprintf('%04d',i)];
      pnfn = fullfile(newpath, fn);
%       pnfn = fullfile([newpath filesep '1_' sprintf('%04d',i)]);    % corrected
